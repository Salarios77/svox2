...
__device__ __inline__ void trace_ray_ray_params_backward(
        const PackedSparseGridSpec& __restrict__ grid,
        ...
        float sparsity_loss,
        PackedGridOutputGrads& __restrict__ grads,
        SingleRayGrad& __restrict__ ray_grad,
        float* __restrict__ accum_out,
        float* __restrict__ log_transmit_out
        ) {
    const uint32_t lane_colorgrp_id = lane_id % grid.basis_dim;
...
#pragma unroll 3
        ...
        if (sigma > opt.sigma_thresh) {
            ...
            float curr_grad_dir[3] = {};
            float curr_grad_origin[3] = {};

            // TODO: voxel size? (x1-x0, y1-y0, z1-z0)
            float dpos_dx[3] = {1.0, 1.0, 1.0};

            float dcolor_dpos[3] = {};
            float dsigma_dpos[3] = {};

            // dc_dzd = -c0 + c1;
            // dc_dyd = (1-zd) * (-c00 + c10) + zd * (-c01 + c11)
            // dc_dxd = (1-zd) * (1-yd) * (-c000 + c100)
            //        + (1-zd) * yd * (-c010 + c110)
            //        + zd * (1-yd) * (-c001 + c101)
            //        + zd * yd * (-c011 + c111)


            trilerp_backward_cuvol_pos(grid.links, grid.sh_data,
                    grid.stride_x,
                    grid.size[2],
                    grid.sh_data_dim,
                    ray.l, ray.pos, lane_id,
            ...
            trilerp_backward_cuvol_pos(grid.links, grid.density_data,
                    grid.stride_x,
                    grid.size[2],
                    1,
                    ray.l, ray.pos, 0,
            ...
            for (int j = 0; j < 3; ++j) {
                dcolor_dx[j] = dcolor_dpos[j] * dpos_dx[j];
            ...
            for (int j = 0; j < 3; ++j) {
                ray_grad.grad_dir[j] += curr_grad_dir[j];
                ray_grad.grad_origin[j] += curr_grad_origin[j];
            }
...
__launch_bounds__(TRACE_RAY_BKWD_CUDA_THREADS, MIN_BLOCKS_PER_SM)
__global__ void render_ray_ray_params_backward_kernel(
    PackedSparseGridSpec grid,
    const float* __restrict__ grad_output,
    const float* __restrict__ color_cache,
    PackedRaysSpec rays,
    ...
    float sparsity_loss,
    PackedGridOutputGrads grads,
    PackedRayOutputGrads ray_grads,
    float* __restrict__ accum_out = nullptr,
    float* __restrict__ log_transmit_out = nullptr) {
    ...
    __shared__ SingleRayGrad single_ray_grad[TRACE_RAY_BKWD_CUDA_RAYS_PER_BLOCK];
    __shared__ typename WarpReducef::TempStorage temp_storage[
        TRACE_RAY_CUDA_RAYS_PER_BLOCK];
    ray_spec[ray_blk_id].set(rays.origins[ray_id].data(),
                             rays.dirs[ray_id].data());
    
    const float zero_array[3] = {0.f, 0.f, 0.f};
    single_ray_grad[ray_blk_id].set(zero_array, zero_array); 
...
#pragma unroll 3
    ...
    trace_ray_ray_params_backward(
        ...
        // output
        grads,
        single_ray_grad[ray_blk_id],
        accum_out == nullptr ? nullptr : accum_out + ray_id,
        log_transmit_out == nullptr ? nullptr : log_transmit_out + ray_id);


    for (int j = 0; j < 3; ++j) {
        ray_grads.grad_origin_out[ray_id*3 + j] = single_ray_grad[ray_blk_id].grad_origin[j];
        ray_grads.grad_dir_out[ray_id*3 + j] = single_ray_grad[ray_blk_id].grad_dir[j];
    }  
}
...
void volume_render_ray_params_backward(
        SparseGridSpec& grid,
        RaysSpec& rays,
        RenderOptions& opt,
        torch::Tensor grad_out,
    ...
    device::render_ray_ray_params_backward_kernel<<<blocks,
